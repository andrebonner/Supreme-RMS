...
if (isset($_GET['addform']))
{
	...
	{
		$error = 'Error adding country to database' . mysqli_error($link);
		include $_SERVER['DOCUMENT_ROOT'] . '/includes/error.html.php';
		exit();
	}
...
if (isset($_POST['action']) and $_POST['action'] == 'Edit')
{
	...
	$sql = "SELECT id, name, status_id FROM countries WHERE id='$id'";
	$result = mysqli_query($link, $sql);
	if (!$result)
	{
		$error = 'Error fetching countries from database.' . mysqli_error($link);
		include $_SERVER['DOCUMENT_ROOT'] . '/includes/error.html.php';
		exit();
	}
...
if (isset($_POST['action']) and $_POST['action'] == 'Delete')
{
	...
	if (!mysqli_query($link, $sql))
	{
		$error = 'Error deleting country from database.';
		include $_SERVER['DOCUMENT_ROOT'] . '/includes/error.html.php';
		exit();
	}
...
$result = mysqli_query($link, 'SELECT countries.id, countries.name, 
status_id, status FROM countries INNER JOIN statustype ON status_id = statustype.id');
...
while ($row = mysqli_fetch_array($result))
{
	$countries[] = array('id' => $row['id'], 'name' => $row['name'], 'status' => $row['status']);
}
